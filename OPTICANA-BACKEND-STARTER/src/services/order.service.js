...
// This powers the lightweight store manager summary without changing the
// existing full admin orders endpoint or its response contract.
export async function listManagerOrders({ recentOnly = true } = {}) {
  ...
  const order =
    await prisma.$transaction(
      async (tx) => {
        ...
        const normalizedInitialPayment = Number(initialPaymentAmount || 0);
        if (!Number.isFinite(normalizedInitialPayment) || normalizedInitialPayment < 0 || normalizedInitialPayment > total) {
          const error = new Error("Invalid initial payment amount");
          error.statusCode = 400;
          error.code = "INVALID_INITIAL_PAYMENT";
          throw error;
        }

        let normalizedDueDate = null;
        if (dueDate) {
          const parsedDueDate = new Date(`${dueDate}T23:59:59.999`);
        ...
        const isPos = String(source || "admin").toLowerCase() === "pos";
        const plannedRemaining = Math.max(total - normalizedInitialPayment, 0);

        if (isPos && plannedRemaining > 0) {
          if (!resolvedCustomerId) {
            const error = new Error("A registered customer is required for credit sales");
            error.statusCode = 400;
            error.code = "CUSTOMER_REQUIRED_FOR_DEBT";
            throw error;
          }
          ...
          if (!customerRecord?.name?.trim() || !customerRecord?.phone?.trim()) {
            const error = new Error("Customer name and phone are required for credit sales");
            error.statusCode = 400;
            error.code = "CUSTOMER_CONTACT_REQUIRED_FOR_DEBT";
            throw error;
          }

          if (!normalizedDueDate) {
            const error = new Error("Due date is required for credit sales");
            error.statusCode = 400;
            error.code = "DUE_DATE_REQUIRED_FOR_DEBT";
            throw error;
          }
        }

        if (normalizedDueDate && normalizedDueDate.getTime() < Date.now()) {
          const error = new Error("Due date cannot be in the past");
          error.statusCode = 400;
          error.code = "DUE_DATE_IN_PAST";
          throw error;
        }
        ...
        if (normalizedInitialPayment > 0) {
          await recordPayment({
            ...
            source: String(source || "admin").toLowerCase(),
            customerId: resolvedCustomerId || null,
            orderId: createdOrder.id,
            createdById: userId || null,
            note: "Initial payment recorded during order creation",
          });
        }
