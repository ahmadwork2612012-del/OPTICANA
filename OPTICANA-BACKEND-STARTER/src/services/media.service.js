...
export async function persistDataUrl(value, mimeType="image/jpeg"){
  ...
  if (!allowedMimeTypes.has(mime)) throw Object.assign(new Error("Unsupported image type"),{statusCode:400,code:"INVALID_MEDIA_TYPE"});
  const encoded = match[2].replace(/\s/g, "");
  if (!encoded || encoded.length % 4 !== 0 || !/^[a-z0-9+/]*={0,2}$/i.test(encoded)) {
    throw Object.assign(new Error("Invalid image encoding"),{statusCode:400,code:"INVALID_MEDIA"});
  }
