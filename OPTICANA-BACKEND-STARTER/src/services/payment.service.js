...
export async function recordPayment({
  ...
  if (orderId) {
    ...
    */
    ...
    const remainingAfterPayment = Math.max(remaining - numericAmount, 0);
    if (String(order.source || "").toLowerCase() === "pos" && remainingAfterPayment > 0 && !order.dueDate) {
      throw createError(
        "A due date is required for POS credit sales",
        400,
        "DUE_DATE_REQUIRED_FOR_DEBT"
      );
    }
  }
