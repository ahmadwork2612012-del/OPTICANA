...
export async function adminCreateProduct(req, res, next) {
  ...
  req.user?.id
    ...
    if (error?.code === "P2002") {
      ...
      const duplicate = target.includes("sku") ? "SKU" : target.includes("slug") ? "slug" : "product field";
      error.statusCode = 409;
      error.code = "DUPLICATE_PRODUCT_FIELD";
      error.message = `${duplicate} already exists`;
    }
    next(error);
  }
}
