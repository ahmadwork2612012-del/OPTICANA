...
export async function adminDeleteCategory(req, res, next) {
  try {
    const result = await deleteCategoryOnly(req.params.id);
...
export async function adminDeleteCategoryWithProducts(req, res, next) {
  try {
    const result = await deleteCategoryWithProducts(req.params.id);
    res.json({ success: true, data: result });
  } catch (error) {
    next(error);
  }
