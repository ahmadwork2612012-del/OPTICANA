...
export async function adminCreateOrder(req, res, next) {
  try {
    const requestedSource = String(req.body?.source || "admin").toLowerCase();
    if (requestedSource === "pos" && req.user?.role === "STAFF") {
      const error = new Error("Staff members are not allowed to use the admin POS");
      error.statusCode = 403;
      error.code = "POS_ACCESS_FORBIDDEN";
      throw error;
    }
