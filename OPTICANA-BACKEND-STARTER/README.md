...
## Fresh delivery database

On a new PostgreSQL database, run the migrations and then `npm run prisma:seed` with `ADMIN_EMAIL` and `ADMIN_PASSWORD` set. The seed creates only the administrator account; products, customers, suppliers, orders, purchases, payments, repairs, expenses, reviews, notifications, settings and CMS content remain empty. The Admin and Store contain safe UI defaults until those values are saved to PostgreSQL.
